...
(*------------------------------------------------lexical address----------------------------------------------------*)
  ...
  match pe, in_tail with 
    (* | ScmSeq'(list), _ -> ScmSeq'(mapping_exp list in_tail)
    | ScmOr'(list) ,_ -> ScmOr'(mapping_exp list in_tail) *)
    ...
    (* and mapping_exp pe in_tail = 
      match pe with
    | [] -> []
    | first :: [] -> (run first in_tail) :: []
    | first :: rest -> (run first in_tail) :: (mapping_exp rest in_tail) *)
   in 
   run pe false ;;
