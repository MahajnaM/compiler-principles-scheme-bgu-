  ...
  let misc_ops =
    let misc_parts = [
	 ...
	 mov rdx, rax
         ...
         "SKIP_TYPE_TAG rax, rsi", make_unary, "car";
         ...
         mov rax, SOB_VOID_ADDRESS", make_binary, "set_cdr";] in
    String.concat "\n\n" (List.map (fun (a, b, c) -> (b c a)) misc_parts);;

  let apply_op_body = "

       mov rsi, [rbp +  WORD_SIZE*3]
       dec rsi
       mov rsi, PVAR(rsi)
       ...
       PUSH_TO_STACK:
         cmp byte[rsi], T_NIL
         je END_PUSH_TO_STACK
         SKIP_TYPE_TAG rdx, rsi
         push rdx
         mov rsi, qword [rsi + TYPE_SIZE + WORD_SIZE]
         inc rbx
         push rcx
         pop rcx
         jmp PUSH_TO_STACK
       ...
       REVERSE_ARGS:
         cmp rcx, rdx
         jae END_REVERSE_ARGS
         mov rax, [rsp + WORD_SIZE * (rbx)]
         mov rax, [rsp + WORD_SIZE * (rbx)]
         mov rdi,[rsp+WORD_SIZE*rcx]
         mov [rsp + WORD_SIZE * rbx], rdi
       ...
       END_REVERSE_ARGS:
         ...
         push rsi
         mov rax, PVAR(0)
         SKIP_TYPE_TAG rdx, rax
         push rdx
         mov rdx, qword [rax + TYPE_SIZE + WORD_SIZE]
         call rdx
         add rsp, WORD_SIZE
         pop rdx
         shl rdx, 3
         ...
         "
let apply_op = make_routine "apply" apply_op_body;;
