...
(define fold-left
	(lambda (curr acc list)
		(if (equal? list '())
			acc
			(fold-left curr (curr acc (car list)) (cdr list)))))

(define fold-right
	(lambda (curr acc list)
		(if (equal? list '())
			acc
		(curr (car list) (fold-right curr acc (cdr list))))))
...
(define cons*
	(lambda (head . tail)
		(if(equal? '() head) 
			'()
		(cons2* (append (list head) tail)))))

(define cons2*
	(lambda (lst)
		(if(equal? '() (cdr lst))
		 	(car lst)
		(cons (car lst) (cons2* (cdr lst))))))
